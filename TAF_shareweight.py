...
def MGC(args,train_set,test_set,source1_loader,source2_loader,source3_loader,target_train_loader,class_num,class_num_test,f):
    ...
    da_net = DAsonNet1(embedding_size,args.son_embeding).cuda()
    ...
    total_model1 = nn.Sequential(base_network,da_net, label_predictor1)#NN网络
    total_model2 = nn.Sequential(base_network,da_net, label_predictor2)  # NN网络
    total_model3 = nn.Sequential(base_network,da_net,label_predictor3)  # NN网络
    ...
    parameter_list = base_network.get_parameters()+ da_net.get_parameters() \
    ...
    for epoch in range(args.num_iterations):#####50010
        if epoch % (20) == 0 and epoch != 0:#取余数 #2500  500  250
            ...
            feature_targets1 = da_net.forward(feature_target)
            feature_targets2 = da_net.forward(feature_target)
            feature_targets3 = da_net.forward(feature_target)
            ...
            with torch.no_grad():#表明当前计算不需要反向传播，使用之后，强制后边的内容不进行计算图的构建
                ...
                code_arr_t1 = da_net.forward(code_arr_t)
                code_arr_t2 = da_net.forward(code_arr_t)
                code_arr_t3 = da_net.forward(code_arr_t)
            ...
            # 保存target预测结果
            # pd.DataFrame(adata2.obs['celltype'].values).to_csv("pancreas2/celltype"+str(epoch)+"result.csv")
        ...
        feature_sources1=da_net.forward(feature_source1)
        feature_targets1 = da_net.forward(feature_target)
        feature_sources2=da_net.forward(feature_source2)
        feature_targets2 = da_net.forward(feature_target)
        feature_sources3=da_net.forward(feature_source3)
        feature_targets3 = da_net.forward(feature_target)
